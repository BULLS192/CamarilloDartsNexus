...
export function normalizeRobustnessPlayer(record={}){
  ...
  return{
    ...raw,
    id:String(raw?.id??record?.player_id??record?.playerId??record?.id??''),
    name:raw?.name??record?.display_name??record?.name??'',
    bullshooter:{...bull,...(bsId?{id:bsId}:{})},
    edc,
    camarillo
  };
}

export function playerBullshooterId(record={}){
  return normalizeRobustnessPlayer(record).bullshooter?.id||'';
}
...
export function bullshooter501Games(b={}){
  return preferredCount(
    ...
    b?.recent01Count,
    Array.isArray(b?.recent01Games)?b.recent01Games.length:0
  );
}
...
export function bullshooterCricketGames(b={}){
  return preferredCount(
    ...
    Array.isArray(b?.recentCricketGames)?b.recentCricketGames.length:0
  );
}
...
export function scorePlayerRobustness(record,{tocLink=null}={}){
  ...
  return{
    score,label:robustnessLabel(score),sources:Object.values(flags).filter(Boolean).length,flags,
    components:{edc:Math.round(edcPoints*10)/10,toc:tocPoints,bullshooter501:Math.round(bs501Points*10)/10,bullshooterCricket:Math.round(bsCricketPoints*10)/10},
...
export function robustnessIndex(records,{links=[]}={}){
  ...
  for(const record of records||[]){
    const p=normalizeRobustnessPlayer(record),playerId=String(p.id||''),bullshooterId=String(p.bullshooter?.id||'');
    const link=linkByPlayer.get(playerId)||null,r=scorePlayerRobustness(p,{tocLink:link});
    const entry={playerId,bullshooterId,name:p.name||'',...r};
    if(playerId)byPlayerId[playerId]=entry;
    if(bullshooterId)byBullshooterId[bullshooterId]=entry;
  }
