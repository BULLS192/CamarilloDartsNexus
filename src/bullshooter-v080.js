...
const PARSER_VERSION='0.8.1';
const BASE='https://www.bullshooter.live/search';
const headers={accept:'application/json,text/plain,*/*','user-agent':'Mozilla/5.0 CamarilloDarts/0.11.3'};
...
async function games(id,discipline,limit=50){
  ...
  for(let attempt=1;attempt<=3;attempt++){
    try{
      const p=await getJson(url);
      if(!p?.success)throw new Error('BullShooter games response unsuccessful');
      if(!Array.isArray(p.games))throw new Error('BullShooter games response missing rows');
      ...
      return {url,rows,available:true,attempts:attempt};
    }catch(error){
      lastError=error;
      if(attempt<3)await sleep(250*attempt);
    }
  }
  return {url,rows:[],available:false,error:lastError?.message||'BullShooter games fetch failed',attempts:3};
}
...
export async function importBullshooterProfile(id){
  ...
  profile.recent01Games=x01.rows.slice(0,50);
  profile.recentCricketGames=cricket.rows.slice(0,50);
  ...
  profile.directGameApiDiagnostics={x01:{url:x01.url,rowCount:x01.rows.length,error:x01.error||null,attempts:x01.attempts||1},cricket:{url:cricket.url,rowCount:cricket.rows.length,error:cricket.error||null,attempts:cricket.attempts||1}};
