...
export function bullshooter501Games(b={}){
  return maxKnown(b?.currentStatsDiagnostics?.x01Count,b?.last50PPDSampleSize,b?.last20PPDSampleSize,b?.last10PPDSampleSize,b?.recent01Count);
}
...
export function scorePlayerRobustness(player,{tocLink=null}={}){
  ...
  const score=Math.round(clamp(edcPoints+tocPoints+bs501Points+bsCricketPoints,0,100)),label=robustnessLabel(score),cdGames=nonneg(c.games01)+nonneg(c.gamesCricket);
  ...
  return{score,label,sources:Object.values(flags).filter(Boolean).length,flags,components:{edc:Math.round(edcPoints*10)/10,toc:tocPoints,bullshooter501:Math.round(bs501Points*10)/10,bullshooterCricket:Math.round(bsCricketPoints*10)/10},evidence:{edcGames,bs501Games,bsCricketGames}};
}
export function robustnessIndex(players,{links=[]}={}){
  ...
  for(const p of players||[]){const link=linkByPlayer.get(String(p?.id))||null,r=scorePlayerRobustness(p,{tocLink:link}),entry={playerId:String(p?.id||''),bullshooterId:String(p?.bullshooter?.id||p?.bullshooterId||''),name:p?.name||'',...r};if(entry.playerId)byPlayerId[entry.playerId]=entry;if(entry.bullshooterId)byBullshooterId[entry.bullshooterId]=entry}
  return{generatedAt:new Date().toISOString(),formulaVersion:'0.9.20',byBullshooterId,byPlayerId,count:Object.keys(byPlayerId).length};
}
export function playersFromRawState(state){return Array.isArray(state?.players)?structuredClone(state.players):[]}
