...
begin
  ...
  with base as (
    ...
    select *,
      case when ((bs50_ppd is not null)::int*0.5 + (bs20_ppd is not null)::int*0.3 + (bs10_ppd is not null)::int*0.2)>0
        then (coalesce(bs50_ppd*0.5,0)+coalesce(bs20_ppd*0.3,0)+coalesce(bs10_ppd*0.2,0))/
             ((bs50_ppd is not null)::int*0.5 + (bs20_ppd is not null)::int*0.3 + (bs10_ppd is not null)::int*0.2)
        else bs_current_ppd end as bs_ppd,
      case when ((bs50_mpr is not null)::int*0.5 + (bs20_mpr is not null)::int*0.3 + (bs10_mpr is not null)::int*0.2)>0
        then (coalesce(bs50_mpr*0.5,0)+coalesce(bs20_mpr*0.3,0)+coalesce(bs10_mpr*0.2,0))/
             ((bs50_mpr is not null)::int*0.5 + (bs20_mpr is not null)::int*0.3 + (bs10_mpr is not null)::int*0.2)
    ...
    select *,
      ...
      case when cd_ppd is not null then least(cd_01_games,30)/30*20 else 0 end as w_cd_ppd,
      case when cd_mpr is not null then least(cd_cricket_games,30)/30*20 else 0 end as w_cd_mpr
    from established
  ), consensus as (
    select *,
    ...
    select *,
      case when nexus_ppd is not null then greatest(0,least(100,(nexus_ppd-10)/25*100)) end as ppd_norm,
      case when nexus_mpr is not null then greatest(0,least(100,(nexus_mpr-1)/3.5*100)) end as mpr_norm,
      case when bs_ppd is not null then greatest(0,least(100,(bs_ppd-10)/25*100)) end as bs_ppd_norm,
      case when bs_mpr is not null then greatest(0,least(100,(bs_mpr-1)/3.5*100)) end as bs_mpr_norm,
      case when edc_ppd is not null then greatest(0,least(100,(edc_ppd-10)/25*100)) end as edc_ppd_norm,
      case when edc_mpr is not null then greatest(0,least(100,(edc_mpr-1)/3.5*100)) end as edc_mpr_norm,
      case when toc_ppd is not null then greatest(0,least(100,(toc_ppd-10)/25*100)) end as toc_ppd_norm,
      case when toc_mpr is not null then greatest(0,least(100,(toc_mpr-1)/3.5*100)) end as toc_mpr_norm,
      case when cd_ppd is not null then greatest(0,least(100,(cd_ppd-10)/25*100)) end as cd_ppd_norm,
      case when cd_mpr is not null then greatest(0,least(100,(cd_mpr-1)/3.5*100)) end as cd_mpr_norm
    from consensus
  ), final as (
    select *,
      case when ppd_norm is not null and mpr_norm is not null then round((ppd_norm+mpr_norm)/2,1) when ppd_norm is not null then round(ppd_norm,1) when mpr_norm is not null then round(mpr_norm,1) end as nexus_rating,
      case when bs_ppd_norm is not null and bs_mpr_norm is not null then round((bs_ppd_norm+bs_mpr_norm)/2,1) when bs_ppd_norm is not null then round(bs_ppd_norm,1) when bs_mpr_norm is not null then round(bs_mpr_norm,1) end as bs_rating,
      case when edc_ppd_norm is not null and edc_mpr_norm is not null then round((edc_ppd_norm+edc_mpr_norm)/2,1) when edc_ppd_norm is not null then round(edc_ppd_norm,1) when edc_mpr_norm is not null then round(edc_mpr_norm,1) end as edc_rating,
      case when toc_ppd_norm is not null and toc_mpr_norm is not null then round((toc_ppd_norm+toc_mpr_norm)/2,1) when toc_ppd_norm is not null then round(toc_ppd_norm,1) when toc_mpr_norm is not null then round(toc_mpr_norm,1) end as toc_rating_norm,
      case when cd_ppd_norm is not null and cd_mpr_norm is not null then round((cd_ppd_norm+cd_mpr_norm)/2,1) when cd_ppd_norm is not null then round(cd_ppd_norm,1) when cd_mpr_norm is not null then round(cd_mpr_norm,1) end as cd_rating
    from scored
  )
  select jsonb_build_object(
    'generatedAt',now(),'formulaVersion','1.0.0','count',count(*),
    'byBullshooterId',coalesce(jsonb_object_agg(bullshooter_id,jsonb_build_object(
      ...
      'sourceRatings',jsonb_build_object('bullshooter',bs_rating,'edc',edc_rating,'toc',toc_rating_norm,'camarillo',cd_rating),
