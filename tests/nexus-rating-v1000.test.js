...
assert.equal(kr.rating,34.9,'Kevin production-shaped BullShooter evidence must rate 34.9');
...
assert.equal(lr.rating,68.2,'Laine multi-source production-shaped rating must be 68.2');
...
assert.equal(jr.rating,93.5,'Joiely BS + verified TOC production-shaped rating must be 93.5');
...
assert.equal(gr.rating,79.7,'confirmed EDC with unknown games receives only provisional 10 total weight');
...
assert.match(ui,/NX.*entry\.rating/s,'runtime must display the Nexus Rating value');
...
assert.match(store,/rpc\/camarillo_nexus_rating_index/,'server transport must call the dedicated Nexus Rating RPC');
assert.match(server,/\/api\/players\/nexus-rating[\s\S]{0,200}getNexusRatingIndexSql/,'server must expose Nexus Rating endpoint');
assert.match(html,/v1000-rating\.js\?v=0\.10\.0/,'final page must load the V0.10.0 rating runtime with cache busting');
assert.match(html,/v1000-rating\.css\?v=0\.10\.0/,'final page must load V0.10.0 rating styling');
const marker=JSON.parse(read('.v1000-rating-applied'));assert.equal(marker.version,'0.10.0');assert.equal(marker.source,'camarillo_nexus_rating_index');assert.equal(marker.route,'/api/players/nexus-rating');
assert.equal(pkg.version,'0.10.0');
console.log('V0.10.0 Nexus Rating contract passed: Kevin 34.9, Laine 68.2, Joiely 93.5; SQL endpoint and single existing rating column wired.');
