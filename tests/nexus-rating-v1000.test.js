...
assert.equal(lr.rating,60.1,'Laine multi-source production-shaped rating must be 60.1');
assert.equal(lr.evidenceWeight,90,'Laine external evidence weight must be BS40 + EDC30 + TOC20');
assert.deepEqual(lr.sourceWeights,{bullshooter:40,edc:30,toc:20,camarillo:0});
...
assert.equal(jr.rating,76.5,'Joiely BS + verified TOC production-shaped rating must be 76.5');
assert.equal(jr.evidenceWeight,60);
...
console.log('V0.10.0+ Nexus Rating contract passed: native PPD + 10*MPR scale, BS40 EDC30 TOC20 CD20; Kevin 41.3, Laine 60.1, Joiely 76.5.');
