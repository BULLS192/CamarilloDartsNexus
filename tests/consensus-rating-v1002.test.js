...
assert.equal(pkg.version,'0.11.0','package must identify the Nexus v2 release');
...
const marker=JSON.parse(read('.v1002-consensus-rating-applied'));assert.equal(marker.version,'0.11.0');assert.equal(marker.ratingFormulaVersion,'2.0.0');assert.equal(marker.agreementBonus,false);assert.deepEqual(marker.windowWeights,{bs50:.7,bs20:.2,bs10:.1});assert.deepEqual(marker.sourcePriorities,{bullshooter:40,edc:30,toc:20,camarillo:50});assert.equal(marker.camarilloZeroUntilGames,true);assert.equal(marker.duplicateTocFactor,.35);
console.log('V0.11.0 consensus deployment contract passed: evidence affects source influence, agreement does not inflate skill, and Camarillo begins at zero.');
