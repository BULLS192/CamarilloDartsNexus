...
const kr=scorePlayerRobustness(kevin),dr=scorePlayerRobustness(derek);assert.ok(kr.score>0&&kr.label!=='','BullShooter-only players must receive a real robustness score');assert.ok(dr.score>=kr.score,'larger verified BullShooter samples should not reduce robustness');
