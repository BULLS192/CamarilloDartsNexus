...
assert.match(ui,/\/api\/players\/robustness/);assert.match(ui,/function syncHiddenColumns\(table\)/,'V0.9.16 renderer source must retain hidden-column repair contract');assert.match(ui,/actionCell\(row\)/);assert.match(ui,/window\.CDNexusRobustnessV0915=exported/,'V0.9.16+ renderer source must satisfy old delegate calls');
assert.match(html,/v091(?:6-robustness|7-table)\.js/,'a V0.9.16-or-later robustness renderer must load');assert.doesNotMatch(html,/v0915-robustness\.js/,'old competing robustness renderer must not load');
const [maj,min,patch]=String(pkg.version).split('.').map(Number);assert.ok(maj>0||min>9||(min===9&&patch>=16),'package must be V0.9.16 or later');
