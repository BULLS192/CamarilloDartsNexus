...
const need=(s,n,label)=>{if(!s.includes(n))throw new Error(`V0.9.21 patch: ${label} anchor not found`)};
...
server=server.replaceAll("version:'0.9.17'","version:'0.9.21'").replaceAll('Camarillo Darts V0.9.17 running','Camarillo Darts V0.9.21 running');
fs.writeFileSync(paths.server,server);

let player=fs.readFileSync(paths.player,'utf8');
player=player.replaceAll('EXTERNAL PLAYER INTELLIGENCE · V0.9.17','EXTERNAL PLAYER INTELLIGENCE · V0.9.21');
