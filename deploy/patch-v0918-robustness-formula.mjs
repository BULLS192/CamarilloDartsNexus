...
// The general listPlayers surface may be normalized/projected. Robustness needs the
// authoritative state objects containing BullShooter counts and confirmed EDC snapshots.
...
server=server.replace(oldRoute,"if(url.pathname==='/api/players/robustness'&&req.method==='GET'){const players=await listPlayersForRobustness(),links=await listTocLinks().catch(()=>[]);return json(res,200,robustnessIndex(players,{links}))}");
...
html=html.replaceAll('V0.9.17','V0.9.19').replaceAll('Camarillo Darts Nexus 0.9.17','Camarillo Darts Nexus 0.9.19');
fs.writeFileSync(paths.index,html);

const pkg=JSON.parse(fs.readFileSync(paths.pkg,'utf8'));
pkg.version='0.9.19';
pkg.description='Camarillo Darts Nexus fixed robustness formula using authoritative player state';
