...
toc=toc.replace(crawlAnchor,"const crawl=await crawlBestKnownSharded({resumeQueue,onProgress:async p=>{if(!(await acquireTocAtomicLease(runId)))throw new Error('TOC atomic sync lease lost during crawl checkpoint.');");
...
const leaseStart=`export async function startTocSync(){\n  if(syncJob.running) return {...syncJob,alreadyRunning:true};\n  const previous=await lastRun().catch(()=>null);\n  const resumeQueue=Array.isArray(previous?.metadata?.resumeQueue)?previous.metadata.resumeQueue:null;\n  const runId=shortId('tocrun',\`${'${Date.now()}|${crypto.randomUUID()}'}\`);\n  if(REMOTE){\n    const acquired=await acquireTocAtomicLease(runId);\n    if(!acquired){\n      return {running:true,runId:previous?.run_id||previous?.runId||null,status:'running',alreadyRunning:true,atomicLease:true,lastProgressAt:previous?.metadata?.lastProgressAt||null,resumeQueueLength:resumeQueue?.length||0};\n    }\n    if(previous?.status==='running'&&(previous.run_id||previous.runId)){\n      const staleId=previous.run_id||previous.runId;\n      await remoteFetch(\`camarillo_external_sync_runs?run_id=eq.\${encodeURIComponent(staleId)}\`,{method:'PATCH',headers:{prefer:'return=minimal'},body:{status:'error',finished_at:now(),error:'Stale TOC worker replaced after atomic lease expiry.',metadata:{...(previous.metadata||{}),stale:true,staleDetectedAt:now(),resumeQueue:Array.isArray(previous?.metadata?.resumeQueue)?previous.metadata.resumeQueue:[]}}}).catch(()=>{});\n    }\n  }\n  void runSync(runId,resumeQueue).finally(()=>releaseTocAtomicLease(runId).catch(()=>{}));\n  return {running:true,runId,status:'starting',atomicLease:REMOTE,resumeQueueLength:resumeQueue?.length||0};\n}`;
toc=toc.replace(startAnchor,leaseStart);
fs.writeFileSync(tocPath,toc);

const pkg=JSON.parse(fs.readFileSync(pkgPath,'utf8'));
pkg.version='0.9.13';
pkg.description='Camarillo Darts Nexus atomic resumable TOC shard persistence with restart-safe Supabase leasing';
