...
// terminal shard can be persisted immediately instead of waiting for the full crawl.
...
if(toc.includes(checkpointAnchor))toc=toc.replace(checkpointAnchor,"metadata:{...run.metadata,checkpoint:true,strategy:TOC_SYNC_STRATEGY,lastProgressAt:now(),persistedRows:p.persistedRows||0,queuedShards:p.queuedShards||0,failedShards:p.failedShards||0}");
...
toc=toc.replace(metaAnchor,"metadata:{...run.metadata,complete:crawl.complete,strategy:crawl.strategy||TOC_SYNC_STRATEGY,shardsProcessed:crawl.shardsProcessed||crawl.pages,splitShards:crawl.splitShards||0,persistedRows:crawl.persistedRows||0,failedShards:crawl.failedShards||[],lastProgressAt:now()}");
