...
const routeRe=/if\(url\.pathname==='\/api\/system\/backend'&&req\.method==='GET'\)return json\(res,200,\{backend:databaseBackend\(\),version:'[^']+'\}\);/;
if(!routeRe.test(server)) throw new Error('V0.8 patch: system route anchor not found');
server=server.replace(routeRe,`if(url.pathname==='/api/system/backend'&&req.method==='GET')return json(res,200,{backend:databaseBackend(),version:'0.8.0'});
