...
const need=(s,n,label)=>{if(!s.includes(n))throw new Error(`V0.10.1-0.10.3 patch: ${label} anchor not found`)};
const replaceBetween=(text,startAnchor,endAnchor,replacement,label)=>{
  ...
  if(start<0||end<0||end<=start)throw new Error(`V0.10.1-0.10.3 patch: ${label} anchors not found`);
...
server=server.replaceAll("version:'0.10.0'","version:'0.10.3'").replaceAll('Camarillo Darts V0.10.0 running','Camarillo Darts V0.10.3 running');
...
player=player.replaceAll('EXTERNAL PLAYER INTELLIGENCE · V0.9.23','EXTERNAL PLAYER INTELLIGENCE · V0.10.3');
...
html=html.replaceAll('/v1000-rating.js?v=0.10.0','/v1000-rating.js?v=0.10.3').replaceAll('/v1000-rating.css?v=0.10.0','/v1000-rating.css?v=0.10.3').replaceAll('V0.10.0','V0.10.3').replaceAll('Camarillo Darts Nexus 0.10.0','Camarillo Darts Nexus 0.10.3');
...
fs.writeFileSync(paths.tableMarker,JSON.stringify({version:'0.10.3',renderer:'v1000-rating',schema:['PLAYER','CONTACT','HOME','BULLSHOOTER','BS CURRENT','BS50','BS20','BS10','CAMARILLO','NEXUS RATING','ROBUSTNESS','ACTIONS'],repairsLegacyHeaders:['BS30','RATING'],metricCells:{nexusRating:9,robustness:10,actions:11}})+'\n');
