...
const need=(s,n,label)=>{if(!s.includes(n))throw new Error(`V0.10.1 patch: ${label} anchor not found`)};
const replaceBetween=(text,startAnchor,endAnchor,replacement,label)=>{
  ...
  if(start<0||end<0||end<=start)throw new Error(`V0.10.1 patch: ${label} anchors not found`);
  return text.slice(0,start)+replacement+text.slice(end);
};

// One authoritative SQL payload now carries both Nexus Rating and Robustness.
...
server=server.replaceAll("version:'0.10.0'","version:'0.10.1'")
  .replaceAll('Camarillo Darts V0.10.0 running','Camarillo Darts V0.10.1 running');
fs.writeFileSync(paths.server,server);

// Player Intelligence keeps filtering/tooling duties, but the V0.10 metrics runtime owns Robustness.
...
if(player.includes('function ensureCanonicalRobustnessRuntime(){')){
  player=replaceBetween(
    player,
...
player=player.replaceAll('EXTERNAL PLAYER INTELLIGENCE · V0.9.23','EXTERNAL PLAYER INTELLIGENCE · V0.10.1');
fs.writeFileSync(paths.player,player);

// The old BS/CD rating script may no longer repaint the same cell after the unified runtime.
let legacyRating=fs.readFileSync(paths.legacyRating,'utf8');
legacyRating=replaceBetween(
  legacyRating,
...
// V0.9 robustness runtime is intentionally not active in V0.10.1.
html=html.replace(/<script\s+src=["']\/v0918-table\.js(?:\?[^"']*)?["'][^>]*><\/script>/g,'');
html=html.replaceAll('/v1000-rating.js?v=0.10.0','/v1000-rating.js?v=0.10.1')
  .replaceAll('/v1000-rating.css?v=0.10.0','/v1000-rating.css?v=0.10.1')
  .replaceAll('V0.10.0','V0.10.1')
  .replaceAll('Camarillo Darts Nexus 0.10.0','Camarillo Darts Nexus 0.10.1');
fs.writeFileSync(paths.index,html);

const pkg=JSON.parse(fs.readFileSync(paths.pkg,'utf8'));
pkg.version='0.10.1';
pkg.description='Camarillo Darts Nexus unified SQL-backed Nexus Rating and Robustness player metrics';
if(!pkg.scripts)pkg.scripts={};
for(const cmd of ['node tests/player-metrics-v1001.test.js'])if(!String(pkg.scripts.check||'').includes(cmd))pkg.scripts.check+=(pkg.scripts.check?' && ':'')+cmd;
fs.writeFileSync(paths.pkg,JSON.stringify(pkg,null,2)+'\n');
fs.writeFileSync(paths.marker,JSON.stringify({
  version:'0.10.1',source:'camarillo_player_metrics_index',route:'/api/players/nexus-rating',renderer:'v1000-rating',owns:['nexus-rating','robustness'],legacyRobustnessRuntime:false,legacyRatingWriter:false
})+'\n');
