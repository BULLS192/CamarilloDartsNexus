...
const need=(s,n,label)=>{if(!s.includes(n))throw new Error(`V0.10.1-0.10.4 patch: ${label} anchor not found`)};
const replaceBetween=(text,startAnchor,endAnchor,replacement,label)=>{
  ...
  if(start<0||end<0||end<=start)throw new Error(`V0.10.1-0.10.4 patch: ${label} anchors not found`);
...
server=server.replaceAll("version:'0.10.0'","version:'0.10.4'").replaceAll('Camarillo Darts V0.10.0 running','Camarillo Darts V0.10.4 running');
...
player=player.replaceAll('EXTERNAL PLAYER INTELLIGENCE · V0.9.23','EXTERNAL PLAYER INTELLIGENCE · V0.10.4');
...
html=html.replaceAll('/v1000-rating.js?v=0.10.0','/v1000-rating.js?v=0.10.4').replaceAll('/v1000-rating.css?v=0.10.0','/v1000-rating.css?v=0.10.4').replaceAll('V0.10.0','V0.10.4').replaceAll('Camarillo Darts Nexus 0.10.0','Camarillo Darts Nexus 0.10.4');
fs.writeFileSync(paths.index,html);

const pkg=JSON.parse(fs.readFileSync(paths.pkg,'utf8'));
pkg.version='0.10.4';pkg.description='Camarillo Darts Nexus identity-mapped player metrics rendering with consensus rating and unified robustness';if(!pkg.scripts)pkg.scripts={};
...
fs.writeFileSync(paths.marker,JSON.stringify({version:'0.10.4',source:'camarillo_player_metrics_index',route:'/api/players/nexus-rating',renderer:'v1000-rating',owns:['bs-current','bs50','bs20','bs10','nexus-rating','robustness'],identityOrder:['bullshooter-id','player-id','unique-name'],legacyBullshooterWriter:false,legacyRobustnessRuntime:false,legacyRatingWriter:false})+'\n');
...
fs.writeFileSync(paths.tableMarker,JSON.stringify({version:'0.10.4',renderer:'v1000-rating',schema:['PLAYER','CONTACT','HOME','BULLSHOOTER','BS CURRENT','BS50','BS20','BS10','CAMARILLO','NEXUS RATING','ROBUSTNESS','ACTIONS'],repairsLegacyHeaders:['BS30','RATING'],statCells:{bsCurrent:4,bs50:5,bs20:6,bs10:7,nexusRating:9,robustness:10,actions:11},rowMatch:['bullshooter-id','player-id','unique-name']})+'\n');
