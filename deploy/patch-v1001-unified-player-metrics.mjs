...
// One authoritative SQL payload carries both Nexus Rating and Robustness.
...
server=server.replaceAll("version:'0.10.0'","version:'0.10.2'")
  .replaceAll('Camarillo Darts V0.10.0 running','Camarillo Darts V0.10.2 running');
fs.writeFileSync(paths.server,server);

// Player Intelligence keeps filtering/tooling duties; V0.10 owns Robustness.
...
if(player.includes('function ensureCanonicalRobustnessRuntime(){')){
  player=replaceBetween(player,'function ensureCanonicalRobustnessRuntime(){','ensureCanonicalRobustnessRuntime();',`function ensureCanonicalRobustnessRuntime(){\n    window.CDNexusPlayerMetricsV1001?.refresh?.();\n    window.CDNexusRatingV1000?.refresh?.();\n  }\n  `,'legacy robustness runtime loader');
}
...
// The old BS/CD rating script may no longer repaint the Nexus Rating cell.
...
html=html.replaceAll('/v1000-rating.js?v=0.10.0','/v1000-rating.js?v=0.10.2')
  .replaceAll('/v1000-rating.css?v=0.10.0','/v1000-rating.css?v=0.10.2')
  .replaceAll('V0.10.0','V0.10.2')
  .replaceAll('Camarillo Darts Nexus 0.10.0','Camarillo Darts Nexus 0.10.2');
fs.writeFileSync(paths.index,html);

const pkg=JSON.parse(fs.readFileSync(paths.pkg,'utf8'));
pkg.version='0.10.2';
pkg.description='Camarillo Darts Nexus consensus-aware multi-source rating with unified SQL-backed robustness';
