...
const state={
  session:null,identity:null,memberships:[],organizations:[],roles:[],rolePermissions:[],integrations:[],selected:null,
  memberRows:[],people:[],memberRoles:new Map(),modules:[],platforms:[],templates:[],venueLinks:[],venues:[],venuePlatforms:[],
  competitions:[],seasons:[],teams:[],teamMembers:[],registrations:[],matches:[],matchSides:[],standings:[],view:'overview'
};
...
async function apiWrite(table,method,body,schema='public'){
  if(!state.session?.access_token)throw new Error('No active session.');
  return jsonFetch(`${SUPABASE_URL}/rest/v1/${table}`,{method,headers:{apikey:PUBLISHABLE_KEY,Authorization:`Bearer ${state.session.access_token}`,'Content-Type':'application/json',Accept:'application/json','Accept-Profile':schema,'Content-Profile':schema,Prefer:'return=representation'},body:body===undefined?undefined:JSON.stringify(body)});
}
async function safeApi(path,schema='public'){try{return await api(path,schema)}catch(error){console.warn(`NEXUS read failed: ${schema}.${path}`,error);return[];}}
...
function moduleEnabled(key){if(!key)return true;const row=state.modules.find(m=>m.module_key===key);return row?row.enabled:true;}
function canCreateType(type){return type==='league'?hasPermission('leagues.create'):hasPermission('tournaments.create');}
function canManageType(type){return type==='league'?hasPermission('leagues.manage'):hasPermission('tournaments.manage');}
...
async function loadIdentity(){
  ...
  state.organizations=organizations||[];state.roles=roles||[];state.rolePermissions=rolePermissions||[];state.integrations=integrations||[];state.memberships=memberships||[];
...
async function loadOrganization(){
  ...
  const memberIds=new Set(state.memberRows.map(r=>r.person_id));state.people=(visiblePeople||[]).filter(p=>memberIds.has(p.id));
  ...
  state.modules=modules||[];state.platforms=platforms||[];state.templates=templates||[];state.venueLinks=venueLinks||[];state.competitions=competitions||[];state.teams=teams||[];

  const venueIds=state.venueLinks.map(v=>v.venue_id),compIds=state.competitions.map(c=>c.id),teamIds=state.teams.map(t=>t.id);
  const venueFilter=inFilter(venueIds),compFilter=inFilter(compIds),teamFilter=inFilter(teamIds);
  ...
  state.venues=venues||[];state.venuePlatforms=venuePlatforms||[];state.seasons=seasons||[];state.teamMembers=teamMembers||[];state.registrations=registrations||[];state.matches=matches||[];state.standings=standings||[];
  const matchIds=state.matches.map(m=>m.id),matchFilter=inFilter(matchIds);
...
function renderShell(){
  ...
  if(integration){orgMode.innerHTML=`<span class="pill bridge">MIGRATING</span> Stage ${integration.migration_stage} · Universal NEXUS engine`;legacyLaunch.href=integration.launch_url||'#';legacyLaunch.classList.toggle('hidden',!integration.launch_url)}
  else{orgMode.innerHTML='<span class="pill active">NATIVE</span> Universal NEXUS engine';legacyLaunch.classList.add('hidden')}
...
function renderView(){
  const views={overview:renderOverview,members:renderMembers,competitions:renderCompetitions,leagues:renderLeagues,tournaments:renderTournaments,teams:renderTeams,schedule:renderSchedule,standings:renderStandings,results:renderResults,venues:renderVenues,admin:renderAdmin};
...
function renderCompetitions(){
  ...
  content.innerHTML=`<div class="section-head"><div><h3>Competitions</h3><p>One engine for leagues, tournaments, weekly events, slings and special events.</p></div>${canCreate?'<button class="primary" id="newCompetitionBtn">+ Create competition</button>':''}</div>${competitionRows(state.competitions)}<div id="competitionBuilder"></div>`;
  el('#newCompetitionBtn')?.addEventListener('click',renderCompetitionBuilder);
}

function renderCompetitionBuilder(){
  const allowedTypes=[['league','League'],['tournament','Tournament'],['weekly_event','Weekly Event'],['sling','Sling / Challenge'],['special_event','Special Event']].filter(([type])=>canCreateType(type));
  ...
  const target=el('#competitionBuilder');if(!target)return;
  target.innerHTML=`<div class="card" style="margin-top:18px"><div class="section-head"><div><h3>Create NEXUS competition</h3><p>This creates an organization-scoped draft using the shared engine.</p></div></div><form id="competitionForm" class="grid"><label>Name<input name="name" required maxlength="120" placeholder="e.g. Tuesday Blind Draw"></label><label>Type<select name="competition_type">${allowedTypes.map(([v,l])=>`<option value="${v}">${l}</option>`).join('')}</select></label><label>Dart type<select name="dart_type"><option value="steel">Steel Tip</option><option value="soft">Soft Tip</option><option value="mixed">Mixed</option></select></label><label>Format<select name="competition_mode"><option value="singles">Singles</option><option value="doubles">Doubles</option><option value="team">Team</option><option value="mixed">Mixed</option></select></label><label>Venue<select name="venue_id">${venueOptions}</select></label><label>Platform<select name="platform_key">${platformOptions}</select></label><label>Schedule<select name="schedule_type"><option value="flexible">Flexible / TBD</option><option value="one_time">One Time</option><option value="recurring">Recurring</option><option value="seasonal">Seasonal</option></select></label><label>Day<select name="day_of_week"><option value="">TBD</option>${['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'].map((d,i)=>`<option value="${i}">${d}</option>`).join('')}</select></label><label>Start time<input type="time" name="start_time"></label><label style="display:flex;gap:8px;align-items:center"><input type="checkbox" name="registration_open" style="width:auto"> Registration open</label><div><button class="primary" type="submit">Create draft</button></div></form><div id="competitionFormStatus" class="muted" style="margin-top:10px"></div></div>`;
  el('#competitionForm')?.addEventListener('submit',createCompetition);
}

async function createCompetition(event){
  event.preventDefault();const form=new FormData(event.currentTarget);const type=form.get('competition_type');
  if(!canCreateType(type)){showToast('Your role cannot create this competition type.');return;}
  const payload={organization_id:currentOrg().id,name:String(form.get('name')||'').trim(),competition_type:type,competition_mode:form.get('competition_mode'),dart_type:form.get('dart_type'),status:'draft',schedule_type:form.get('schedule_type'),registration_open:form.get('registration_open')==='on',is_public:true,created_by_person_id:state.identity.id,settings:{allow_public_registration:false}};
  if(form.get('venue_id'))payload.venue_id=form.get('venue_id');if(form.get('platform_key'))payload.platform_key=form.get('platform_key');if(form.get('day_of_week')!=='')payload.day_of_week=Number(form.get('day_of_week'));if(form.get('start_time'))payload.start_time=form.get('start_time');
  const status=el('#competitionFormStatus');if(status)status.textContent='Creating…';
  try{await apiWrite('nexus_competitions','POST',payload);await loadOrganization();renderCompetitions();showToast(`${payload.name} created as a draft.`);}catch(error){if(status)status.textContent=error.message||'Unable to create competition.';}
}

function renderLeagues(){
  const rows=state.competitions.filter(c=>c.competition_type==='league');
  content.innerHTML=`<div class="section-head"><div><h3>Leagues</h3><p>League records use the same NEXUS engine as every other organization.</p></div>${hasPermission('leagues.create')?'<button class="primary" id="newLeagueBtn">+ New league</button>':''}</div>${competitionRows(rows)}<div id="competitionBuilder"></div>`;
  el('#newLeagueBtn')?.addEventListener('click',()=>{renderCompetitionBuilder();const select=el('#competitionForm select[name="competition_type"]');if(select){select.value='league';select.disabled=true;}});
}

function renderTournaments(){
  const rows=state.competitions.filter(c=>c.competition_type!=='league');
  content.innerHTML=`<div class="section-head"><div><h3>Tournaments & events</h3><p>Tournaments, weekly events, slings and special events all use the shared competition engine. Bracket Engine V2 plugs into these records next.</p></div>${hasPermission('tournaments.create')?'<button class="primary" id="newTournamentBtn">+ New event</button>':''}</div>${competitionRows(rows)}<div id="competitionBuilder"></div>`;
  el('#newTournamentBtn')?.addEventListener('click',renderCompetitionBuilder);
}

function renderTeams(){
  content.innerHTML=`<div class="section-head"><div><h3>Teams</h3><p>Organization-scoped teams and captains backed by the universal NEXUS roster model.</p></div></div>${state.teams.length?`<div class="table"><div class="row head"><span>Team</span><span>Competition</span><span>Captain</span><span>Status</span></div>${state.teams.map(t=>`<div class="row"><b>${escapeHtml(t.name)}</b><span>${escapeHtml(competitionName(t.competition_id))}</span><span class="muted">${escapeHtml(personName(t.captain_person_id))}</span><span class="pill ${t.status==='active'?'active':''}">${escapeHtml(titleCase(t.status))}</span></div>`).join('')}</div>`:'<div class="empty">No teams have been created yet.</div>'}`;
}

function renderSchedule(){
  content.innerHTML=`<div class="section-head"><div><h3>Schedule</h3><p>League and tournament matches converge into the same match engine.</p></div></div>${state.matches.length?`<div class="stack">${state.matches.slice(0,60).map(m=>{const sides=sidesFor(m.id);return`<div class="card"><div class="row" style="grid-template-columns:1.4fr .8fr .8fr .7fr;padding:0;border:0"><b>${escapeHtml(sideName(sides[0]))} vs ${escapeHtml(sideName(sides[1]))}</b><span>${escapeHtml(fmtDate(m.scheduled_at))}</span><span class="muted">${escapeHtml(venueName(m.venue_id))}</span><span class="pill ${m.status==='final'?'active':''}">${escapeHtml(titleCase(m.status))}</span></div></div>`}).join('')}</div>`:'<div class="empty">No matches have been generated yet.</div>'}`;
}

function renderStandings(){
  const rows=[...state.standings].sort((a,b)=>(a.rank??9999)-(b.rank??9999)||Number(b.points||0)-Number(a.points||0));
  content.innerHTML=`<div class="section-head"><div><h3>Standings</h3><p>Shared standings records for person- and team-based competitions.</p></div></div>${rows.length?`<div class="table"><div class="row head"><span>Entry</span><span>Played</span><span>Record</span><span>Points</span></div>${rows.map(r=>`<div class="row"><b>${escapeHtml(r.entity_type==='team'?teamName(r.team_id):personName(r.person_id))}</b><span>${escapeHtml(r.played)}</span><span class="muted">${escapeHtml(`${r.wins||0}-${r.losses||0}-${r.draws||0}`)}</span><b>${escapeHtml(r.points)}</b></div>`).join('')}</div>`:'<div class="empty">No standings have been calculated yet.</div>'}`;
}

function renderResults(){
  const finals=state.matches.filter(m=>m.status==='final');
  content.innerHTML=`<div class="section-head"><div><h3>Results</h3><p>Finalized results from the universal match engine.</p></div></div>${finals.length?`<div class="stack">${finals.slice(-40).reverse().map(m=>{const sides=sidesFor(m.id);return`<div class="card"><b>${escapeHtml(sideName(sides[0]))} ${escapeHtml(sides[0]?.score??'—')} – ${escapeHtml(sides[1]?.score??'—')} ${escapeHtml(sideName(sides[1]))}</b><div class="muted">${escapeHtml(competitionName(m.competition_id))} · ${escapeHtml(fmtDate(m.completed_at||m.scheduled_at))}</div></div>`}).join('')}</div>`:'<div class="empty">No finalized results yet.</div>'}`;
}

function renderVenues(){
  content.innerHTML=`<div class="section-head"><div><h3>Venues</h3><p>Global NEXUS venue records linked to ${escapeHtml(currentOrg().name)}. A venue can serve multiple organizations without duplication.</p></div><span class="pill active">${state.venues.length} LINKED</span></div>${state.venues.length?`<div class="table"><div class="row head"><span>Venue</span><span>Location</span><span>Dart systems</span><span>Relationship</span></div>${state.venues.map(v=>{const link=state.venueLinks.find(x=>x.venue_id===v.id);const platforms=state.venuePlatforms.filter(x=>x.venue_id===v.id).map(x=>platformName(x.platform_key));return`<div class="row"><div><b>${escapeHtml(v.name)}</b><div class="muted">${escapeHtml(v.address_line1||'')}</div></div><span>${escapeHtml([v.city,v.state].filter(Boolean).join(', ')||'—')}</span><span class="muted">${escapeHtml(platforms.join(', ')||'Unspecified')}</span><span>${escapeHtml(titleCase(link?.relationship||'listed'))}</span></div>`}).join('')}</div>`:'<div class="empty">No venues are linked to this organization yet. Venue creation/linking controls are the next Organization OS module.</div>'}`;
}

function renderAdmin(){
  const i=currentMembership().integration;
  content.innerHTML=`<div class="section-head"><div><h3>Organization administration</h3><p>NEXUS platform capabilities are shared; these settings belong only to ${escapeHtml(currentOrg().name)}.</p></div></div><div class="grid"><div class="card"><div class="eyebrow">YOUR ACCESS</div><h3>${escapeHtml(roleLabels().join(' · '))}</h3><p class="muted">${escapeHtml([...permissionSet()].sort().join(' · '))}</p></div><div class="card"><div class="eyebrow">ENGINE MODE</div><h3>Universal NEXUS</h3><p class="muted">${i?`TDT migration Stage ${i.migration_stage}; legacy app remains available during write-parity migration.`:'No organization-specific engine is required.'}</p></div></div></div><div class="section-head"><div><h3>Enabled modules</h3><p>Module configuration is organization-level; implementation remains NEXUS-level.</p></div></div><div class="module-grid">${state.modules.map(m=>`<div class="module-card"><b>${escapeHtml(titleCase(m.module_key))}</b><p>${m.enabled?'Enabled for this organization.':'Disabled for this organization.'}</p></div>`).join('')}</div>`;
}

function moduleDescription(key){return({overview:'Organization snapshot, roles and activity.',members:'Canonical NEXUS identities and organization memberships.',competitions:'One engine for every league, tournament, weekly event and challenge.',leagues:'League configuration, seasons, registration and director controls.',tournaments:'Tournament and recurring-event configuration; Bracket Engine V2 connects here.',teams:'Rosters, captains, lineups and team operations.',schedule:'Universal match scheduling with venue and board assignment.',standings:'Standings derived from finalized competition results.',results:'Finalized match results and future score-confirmation workflow.',venues:'Global venue directory, dart platforms and board inventory.',admin:'Organization module configuration, roles and migration status.'})[key]||'';}
...
function sideName(side){if(!side)return'TBD';if(side.participant_type==='team')return teamName(side.team_id);if(side.participant_type==='person')return personName(side.person_id);if(side.participant_type==='bye')return'BYE';return'TBD';}
